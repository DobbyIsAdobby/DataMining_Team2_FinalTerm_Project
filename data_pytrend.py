...
def safe_request(pytrends, kw_list, timeframe, geo='', retries=4): # 시도 횟수를 2에서 4로 증가
    assert len(kw_list) <= 5, "한 요청당 키워드는 최대 5개까지만 가능합니다."
    
    for attempt in range(1, retries + 1):
        # 기본 딜레이도 약간 늘려 애초에 차단당할 확률을 낮춤
        ...
        try:
            ...
            # 429 차단 시 핵심 로직: 시도 횟수에 비례하여 대기 시간을 크게 늘림 (60초 -> 120초 -> 180초)
            if '429' in str(e):
                cool = 60 * attempt 
...
def run_pipeline(en_brand, ko_brand):
    ...
    # 3. 전처리 및 병합 로직 (원본 build_dataset 기반)
    rows = []
