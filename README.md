...
* **Mac/Linux 사용자:**
  ...
  ```
> 터미널 입력창 맨 앞에 `(venv)`라는 글자가 표시되면 정상적으로 활성화된 것입니다.

---

## 3. 필수 패키지 설치
활성화된 가상환경 상태에서 시스템 구동에 필요한 모든 외부 라이브러리를 설치합니다.<br>
`requirements.txt` 파일이 아래 내용으로 작성되어 있는지 확인하세요.
...
SteamDB사이트에 Steam계정으로 로그인 후 수집을 원하는 게임을 검색 후 해당 페이지에서 `f12 -> Console`에 아래 코드를 복사 후 실행하세요.
```javascript
...
아래 3개의 수집 모듈을 실행하여 주가, 검색 트렌드, 스팀 리뷰 데이터를 수집합니다.<br>
`data_yfinance.py` : 수집을 원하는 회사의 주식 Ticker를 반드시 입력해주세요.<br>
`data_pytrend.py` : 수집을 원하는 게임의 정확한 한/영문 이름을 입력해주세요.<br>
`data_steam.py` : 수집을 원하는 게임의 appid와 지역, 출시일, 마지막으로 패치노트 csv 파일명(필수)을 입력해주세요.<br>
```bash
...
```
>  기업명, 앱ID, 영문명, 패치 파일명, **기업 시가총액(억 달러)**을 반드시 입력해주세요.

### Step 3. 리스크 예측 리포트 확인 (최종)
완성된 통합 데이터를 Stacking 모델에 통과시켜 향후 3일 내의 2%이상 하락할 확률을 진단합니다.
```bash
python Risk_Predictor.py
```
> 스크립트가 실행되면 내부적으로 시계열 파생 변수를 자동 연산한 뒤, 어제 발생한 **'핫픽스 횟수'**를 질문합니다. 숫자를 입력하면 데이터 기반의 최적 임계값(Threshold)을 거친 최종 예측 리포트가 출력됩니다.
